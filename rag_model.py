...
# Cache for storing loaded vector data
_vector_cache = {}
...
def _get_teams_hash():
    """
    Generate a hash of all team names to detect changes in team list.
    """
    team_names = mongoclient.get_all_teams()
    teams_str = json.dumps(sorted(team_names))
    return hashlib.md5(teams_str.encode()).hexdigest()
...
def load_vector_db_for_selected_team(team_name):
    """
    Load vector database for a specific team with caching.
    Only reloads if the team data hasn't been loaded before.

    :param team_name: The name of the team to load data for.
    """
    global retrieval_chain, _vector_cache
    
    # Check if this team's data is already cached
    if team_name in _vector_cache.get('loaded_teams', set()):
        logger.info(f"Team {team_name} data already loaded in cache")
        return

    logger.info(f"Loading vector data for team: {team_name}")
    ...
    if 'loaded_teams' not in _vector_cache:
        _vector_cache['loaded_teams'] = set()
    _vector_cache['loaded_teams'].add(team_name)
    
    logger.info(f"Successfully loaded and cached data for team: {team_name}")
